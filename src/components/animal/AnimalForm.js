...
// // 2. useEffect() invoked. For the dropdowns get data for locations and customers.
// // 3. Determine if this is an edit based on animalId in the URL. If true, invoke getAnimalById() and then setAnimal state
// // 4. Render (display new state in DOM)
// // 5. User makes changes. As changes are made, state is updated. The DOM always displays what is in state.
// // 6. Select Save
// // 7. Invoke handleSaveAnimal(). This functions determines if this is a new animal or edit, prepares an 
// //      object with the animal data and invokes the appropriate provider method (addAnimal or updateAnimal).
// // 8. The handleSaveAnimal method will also setIsLoading(true) - this ensures the user cannot 
// //      repeatedly click the button while the API is being updated.
// // 9. Once the API has updated, change the view to display updated data
